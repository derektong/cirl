class LegalResourceSearch < ActiveRecord::Base
  ...
  attr_accessible :free_text, :name, :date_from, :date_to, :day_from, :month_from, 
                  :year_from, :day_to, :month_to, :year_to, :legal_resource_name,  
                  :publisher_ids, :document_type_ids, :process_topic_ids, :child_topic_ids, 
                  :refugee_topic_ids, :keyword_ids

  # handle legal_resource search
  ...
  def description
   ...
   description << "for a legal_resource name containing: \"" + self.legal_resource_name + "\"" if 
    !self.legal_resource_name.empty? 
   description << "for a legal_resource decided between " + self.date_from.strftime("%d %b %Y") + 
                  " and " + self.date_to.strftime("%d %b %Y") if self.date_from && self.date_to 
   description << "published by: \"" + self.publishers.map(&:name).join(' or ') + "\"" if 
                  !self.publishers.empty? 
   description << "with document type: \"" + self.document_types.map(&:name).join(' or ') + "\"" if 
                  !self.document_types.empty? 
